...
try {
    ...
    console.error("A fatal error occurred during application startup:", e);
    const rootElement = document.getElementById('root');
    if (rootElement) {
        const error = e instanceof Error ? e : new Error(String(e));
        rootElement.innerHTML = `
            <div style="font-family: sans-serif; padding: 2rem; background-color: #fef2f2; border: 1px solid #fecaca; border-radius: 8px; color: #991b1b; max-width: 900px; margin: 2rem auto; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                <h1 style="font-size: 1.5rem; font-weight: bold; margin-bottom: 1rem;">어플리케이션을 시작할 수 없습니다.</h1>
                <p style="margin-bottom: 1rem;">페이지를 로드하는 중 심각한 오류가 발생했습니다. 이 오류는 보통 API 키 설정 문제 또는 브라우저 호환성 문제로 인해 발생합니다.</p>
                <div style="background-color: #fee2e2; padding: 1rem; border-radius: 4px; font-family: monospace; white-space: pre-wrap; word-break: break-all; font-size: 0.875rem;">
                    <strong>오류 정보:</strong>
                    <p style="margin-top: 0.5rem;">${error.message}</p>
                    <p style="margin-top: 1rem; color: #7f1d1d; font-size: 0.75rem; max-height: 200px; overflow-y: auto;">${error.stack}</p>
                </div>
                 <p style="margin-top: 1rem; font-size: 0.9rem; color: #555;">
                    <b>문제 해결:</b> 페이지를 새로고침하거나 잠시 후 다시 시도해 주세요. 문제가 계속되면 개발자 도구(F12)의 콘솔 탭에서 더 자세한 정보를 확인할 수 있습니다.
                </p>
            </div>
        `;
    }
